...
export default async function handler(req, res) {
  ...
  try {
    ...
    const doc = new GoogleSpreadsheet(
      '1ZO1WmiTxyhZvHQbehAlNrvwWEzrLirsecRzm74OIjrQ',
      useServiceAccountAuth
    );

    await doc.loadInfo();
    const sheet = doc.sheetsByIndex[0]; // Access the second sheet
    ...
    const isAddressAlreadyListed = rows.some((row) => {
      ...
      return (
        sheetAddress && sheetAddress.trim().toLowerCase() === walletAddressLower
      );
    });
    ...
    if (isAddressAlreadyListed) {
      ...
      res
        .status(200)
        .json({ message: 'Address already exists', addressExists: true });
    } else {
      ...
      sendTelegramMessage(`New presale whitelist request: ${walletAddress}`);
      res
        .status(200)
        .json({ message: 'Address added successfully', addressExists: false });
    }
  } catch (error) {
