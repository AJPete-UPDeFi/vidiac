...
const VidiReferral = () => {
  ...
  const checkWhitelistStatus = async () => {
    ...
    try {
      const response = await fetch(
        `/api/checkWhitelistStatus?walletAddress=${encodeURIComponent(
          userAddress
        )}`,
        {
          method: 'GET',
  ...
  const handleWhitelistApplication = async () => {
    ...
    if (response.ok) {
      if (responseData.addressExists) {
        ...
        setError('');
        console.log(
          'Your address has been added to the whitelist and is awaiting approval.'
        );
        onOpen();
      }
    } else {
  ...
  const shareMessageApply = encodeURIComponent(
    'Thank you for the free VIDI @Vidiac_Token 💵 Get yours by checking out www.vidiac.co 🚀 #UPDeFi #Vidiac #VIDI'
  );
  const twitterShareUrlApply = `https://twitter.com/intent/tweet?text=${shareMessageApply}`;

  const shareMessageWhitelist = encodeURIComponent(
    '@Vidiac_Token I just applied for the referral whitelist and I am excited to join the #Vidiac family! www.vidiac.co #VIDI'
  );
  ...
  useEffect(() => {
    ...
    if (referralReserveData) {
      ...
      const formattedReferralReserve = parseFloat(
        formatEther(referralReserveBigInt)
      );
  ...
  return (
    <Card className="m-4 mb-10 max-w-xl bg-black/60 shadow-lg">
      <CardHeader className="rounded-t-lg bg-transparent">
        <p className="text-2xl font-bold text-white sm:text-4xl">
          Have a Referral Code?
          <p className="py-2 text-xl font-bold sm:text-3xl"> Apply it here!</p>
          <p className="text-lg font-semibold sm:text-2xl">
            Current Reward Amount: {rewardAmount} VIDI
          </p>
          <p className="text-sm font-semibold sm:text-xl">
            Referral Tokens Available: {referralReserve.toFixed(2)} VIDI
          </p>
          <p className="text-xs font-normal sm:text-lg">
            You must have at least 100 VIDI to claim a code.
          </p>
          <button
            data-aos="fade-right"
      ...
      <CardBody className="rounded-2xl bg-gradient-to-t from-brandLavender to-blue-300">
        {!userAddress ? (
          <div className="flex-grid justify-text-center flex justify-between">
          ...
          <>
            <div className="flex flex-col items-center justify-center">
              <input
                ...
                className="w-full max-w-xs rounded-lg border-2 border-gray-300 p-2 text-center text-2xl font-semibold"
              />
              <Button
                  ...
                  {(onClose) => (
                    <>
                      <ModalHeader className="flex flex-col gap-1 text-xl font-bold text-gray-200">
                        The referral code has been applied!
                      </ModalHeader>
                      <ModalBody>
                        <div className="text-lg font-semibold text-gray-200">
                          <p>
                            Enjoy your VIDI tokens. We're excited to have you on
                            board.
                          </p>
                          <Divider className="my-2" />
                          <p>
                            Let the world know you're part of the Vidiac
                            revolution! Share on Twitter:
                          </p>
                        </div>
                      </ModalBody>
                      <ModalFooter>
          ...
          <div>
            <p className="text-xl font-semibold">
              Congratulations, looks like you have already applied a referral
              code. Thank you for supporting Vidiac creators.
            </p>
          </div>
        ) : isAwaitingApproval ? (
          <p className="text-xl font-semibold">
            Please be patient as your address is awaiting approval. Join the
            conversation here
            <a
              href="https://t.me/vidiac_token"
              target="_blank"
              rel="noopener noreferrer"
          ...
          <div>
            <p className="text-md font-semibold">
              Your address is not yet eligible for a referral. For the security
              of the contract, an address must be whitelisted prior to applying
              a referral code.
            </p>
            {error && (
            ...
            {contractWriteError && (
              <p className="text-semibold text-2xl text-red-600">
                Error: {contractWriteError.message}
              </p>
            )}
            <>
              <Button
