...
const Presale = () => {
  ...
  const numericUserTokenBalance = userTokenBalance
  ...
  const handleTokenAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      value === '' ||
      (isWholeNumber(value) && isWithinRange(value, 1, 25000))
    ) {
  ...
  const handlePurchase = () => {
    ...
    try {
      const weiValue = BigInt(
        Math.floor(TOKEN_PRICE_ETH * parseFloat(tokenAmount) * 1e18)
      );
      console.log(
        `Attempting to purchase tokens: ${tokenAmount}, Wei Value: ${weiValue}`
      );
      write({
        args: [tokenAmount],
        value: weiValue,
  ...
  const shareMessage = encodeURIComponent(
    "Exciting times ahead for video enthusiasts! I'm diving into the Vidiac (VIDI) presale – a revolutionary step to uplift indie video creators and secure USD dividends for investors. www.up-defi.com/vidiac 🎥💰 #UPDeFi #Vidiac #VIDI"
  );
  ...
  useEffect(() => {
    ...
    if (writeError) {
      ...
      setTransactionMessage(
        'Transaction failed. Ensure you have enough ETH to cover the transaction fee. Ensure you are following purchase limits.'
      );
    }
  }, [
    presaleActiveData,
  ...
  return (
    <>
      <Card className="max-w-xl bg-black/60 shadow-lg xs:m-4">
        <CardHeader className="rounded-t-lg bg-transparent">
          <p className="text-2xl font-bold text-white sm:text-4xl">
            Calling All Video Maniacs!
            <p className="py-3 text-xl font-semibold sm:text-3xl">
              {' '}
              Earn USD Dividends, Support Video Creators
            </p>
            <p className="py-1 text-xl font-semibold sm:text-3xl">
              {' '}
              Join the Vidiac Private Sale!
            </p>
          </p>
        </CardHeader>
        <CardBody className="rounded-lg bg-gradient-to-t from-blue-50 to-blue-300">
          {!userAddress ? (
            <div>
              <h3 className="text-xl font-bold text-brandBlue">
                Vidiac(VIDI) Private Sale
              </h3>
              <p className="text-lg font-semibold">
                Soft Cap 10 ETH / Hard Cap 20 ETH
              </p>
              <p>
                <AddressCopy />
              </p>
              <p className="text-lg font-semibold">
                Minimum Initial Purchase: 250 VIDI / 0.01 ETH
              </p>
              <p className="text-lg font-semibold">
                Maximum Purchase: 25,000 VIDI / 1 ETH
              </p>
              <p className="text-lg font-semibold">
                Your VIDI Balance: {formatTokenCount(userTokenBalance)} Please
                Connect Wallet
              </p>
              <p className="text-lg font-semibold">
                Tokens Remaining for Private Sale:{' '}
                {totalPresaleSold
                  ? formatTokenCount(
                      (500000 - parseFloat(totalPresaleSold)).toString()
                    )
                  : 'Loading...'}{' '}
                VIDI
              </p>
            ...
            <>
              ...
              <p className="text-lg font-semibold">
                Your VIDI Balance: {formatTokenCount(userTokenBalance)} VIDI
              </p>
              <p className="text-lg font-semibold">
                Tokens Remaining for Private Sale:{' '}
                {totalPresaleSold
              ...
              <div className="flex flex-col items-center justify-center">
                <p className="p-2 text-2xl font-semibold">
                  Thank you for participating in the private sale! You have
                  reached the maximum private sale token amount.
                </p>
              </div>
            </>
            ...
            <>
              ...
              <div className="flex flex-col items-center justify-center">
                <input
                  ...
                  className="w-full max-w-xs rounded-lg border-2 border-gray-300 p-2 text-center text-xl font-semibold"
                />
                <Button
                  onClick={handlePurchase}
                  disabled={!tokenAmount}
                  className="mt-4 max-w-xs text-2xl font-bold"
                  color="primary"
                >
                  Purchase Tokens
                </Button>
            ...
            <div>
              ...
              <p className="mb-1 p-2 text-xl font-semibold">
                Your address is not whitelisted for the private sale.
              </p>
              {error && (
                <p className="text-semibold text-2xl text-red-600">
                  Error: {error}
                </p>
              )}
              {writeError && (
              ...
              <>
                ...
                >
                  <ModalContent>
                    {(onClose) => (
                      <>
                        <ModalHeader className="flex flex-col gap-1 text-xl font-bold text-gray-200">
                          Thank you for your interest!
                        </ModalHeader>
                        <ModalBody>
                          <div className="text-lg font-semibold text-gray-200">
                            <p>
                              Your address will be whitelisted shortly. We're
                              excited to have you on board.
                            </p>
                            <Divider className="my-2" />
                            <p>
                              Spread the word and share your enthusiasm for UP
                              DeFi and Vidiac with your friends and followers!
                        ...
                        <ModalFooter>
                          ...
                          <a
                            ...
                            className="rounded bg-blue-500 px-4 py-2 font-semibold text-white shadow-lg shadow-indigo-500/20 hover:bg-blue-700"
                          >
                            Share on X
                          </a>
                        </ModalFooter>
          ...
          <div className="flex flex-col items-center justify-center sm:flex-row">
            <div className="w-full px-2 sm:w-1/2">
              <p className="mt-4 text-center text-lg font-semibold sm:text-left">
                To show our gratitude, purchase 2,500 VIDI in the private sale
                and receive a limited edition physical coin!
              </p>
              <img
                src="/images/VidiacETHCoinFront.png"
                alt="VidiacETH Coin Front"
                className="mx-auto mt-4 w-full rounded-full"
              />
            </div>
            <div className="w-full px-2 sm:w-1/2">
              <p className="mt-4 text-center text-lg font-semibold sm:text-left">
                Each coin will be personalized with your wallet address and QR
                code that can be used to send or receive crypto!
              </p>
              <img
                src="/images/VidiacETHCoinBack.png"
                alt="VidiacETH Coin Back"
                className="mx-auto mt-4 w-full rounded-full"
              />
            </div>
          </div>
