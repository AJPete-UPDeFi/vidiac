...
const DividendCalculator = () => {
  ...
  const calculateDividends = () => {
    ...
    const dailyDividends =
      ((numTokensOwned / totalCirculatingVolume) *
        (numDailyVolume * (taxRateDecimal * numDividendRatio))) /
      100;
    const weeklyDividends = dailyDividends * 7;
    const annualDividends = dailyDividends * 365;
    const featuredWalletDaily =
      numDailyVolume *
      (taxRateDecimal - taxRateDecimal * (numDividendRatio / 100));
  ...
  const handleDividendRatioChange = (
  ...
  return (
    <div data-aos="fade-up" className="p-4">
      <p className="mb-4 text-center text-5xl font-extrabold text-gray-900">
        Dividend Calculator
      </p>
      <p className="mb-4 text-center text-2xl font-bold text-gray-900">
        Try different values to see your potential earnings!
      </p>
      <div className="mb-4">
        <label
          htmlFor="tokensOwned"
        ...
        <input
          ...
          className="mt-1 block w-full max-w-xs rounded-md border border-gray-300 shadow-sm"
      ...
      <div className="text-xl font-semibold text-gray-700">
        <p className="mb-4 text-2xl font-bold text-gray-900"> Your Dividends</p>
        <div className="flex flex-row text-lg sm:text-2xl">
          Daily USD Dividends:{' '}
          <p className="ml-auto font-bold text-green-700">
            ${dailyDividends.toFixed(2)}
          </p>
        </div>
        <div className="flex flex-row text-lg sm:text-2xl">
          Weekly USD Dividends:{' '}
          <p className="ml-auto font-bold text-green-700">
            ${weeklyDividends.toFixed(2)}
          </p>
        </div>
        <div className="flex flex-row text-lg sm:text-2xl">
        ...
        <p className="mb-4 mt-4 text-2xl font-bold text-gray-900">
          {' '}
          Featured Wallet Dividends
        </p>
        <div className="flex flex-row text-lg sm:text-2xl">
          Daily USD Dividends:{' '}
          <p className="ml-auto font-bold text-green-700">
            ${featuredWalletDaily.toFixed(2)}
          </p>
        </div>
        <div className="flex flex-row text-lg sm:text-2xl">
          Weekly USD Dividends:{' '}
          <p className="ml-auto font-bold text-green-700">
            ${featuredWalletWeekly.toFixed(2)}
          </p>
        </div>
      </div>
    </div>
